...
def compute_loss(output, pred_box, gt_box, gt_mask, num_boxes, num_classes, grid_size, image_size):
    ...
    loss_x = weight_coord * torch.sum(box_mask * torch.pow(gt_box[:, 0:1, :, :] - output[:, 0:5*num_boxes:5], 2.0))
    
    # loss_y: loss function on y coordinate (cy)
    loss_y = weight_coord * torch.sum(box_mask * torch.pow(gt_box[:, 1:2, :, :] - output[:, 1:5*num_boxes:5], 2.0))
    
    # loss_w: loss function on width 
    loss_w = weight_coord * torch.sum(box_mask * torch.pow(torch.sqrt(gt_box[:, 2:3, :, :]) - torch.sqrt(output[:, 2:5*num_boxes:5]), 2.0))
    
    # loss_h: loss function on height
    loss_h = weight_coord * torch.sum(box_mask * torch.pow(torch.sqrt(gt_box[:, 3:4, :, :]) - torch.sqrt(output[:, 3:5*num_boxes:5]), 2.0))
    ...
    loss_noobj = weight_noobj * torch.sum((1.0 - box_mask) * torch.pow(0.0 - output[:, 4:5*num_boxes:5], 2.0))
    
    # loss_cls: loss function for object class
    gt_classes = gt_box[:, 4:, :, :]  # Ground truth class probabilities
    pred_classes = output[:, 5*num_boxes:, :, :]  # Predicted class probabilities
    loss_cls = torch.sum(
        box_mask * torch.pow(gt_classes - pred_classes, 2.0)
    )
